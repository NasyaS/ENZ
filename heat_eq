...
{
    ofstream outStream("res/" + to_string(N) + ".txt", ios_base::out);
    outStream << t << "\n";
...
int main(int argc, char *argv[])
{
    ...
    /*Выделение памяти под трехмерный массив u(x,y,t), где первая размерность указывает на время, т.е 
    ...
    */
    ...
    if (l1 + l2 > 0.5)
    {
        cout << "Stability condition not satisfied!\n";
        return 0;
    }
...
#pragma omp parallel for
    ...
    double calc_time = chrono::duration_cast<chrono::duration<double>>(chrono::steady_clock::now() - begTime).count();

    cout << omp_get_max_threads() << " " << calc_time << endl;

    // вывод решения в файл
    if (write_file)
    {
        experimental::filesystem::remove_all("res");
        experimental::filesystem::create_directory("res");
        for (size_t iT = 0; iT < nT + 1; iT++)
        {
